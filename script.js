...
function moveNo() {
    ...
    // Пределы для границ экрана
    var maxX = window.innerWidth - noElement.clientWidth;
    var maxY = window.innerHeight - noElement.clientHeight;

    // Генерируем новые координаты, удерживая "Нет" внутри экрана
    var newLeft = Math.max(0, Math.min(maxX, Math.random() * maxX));
    var newTop = Math.max(0, Math.min(maxY, Math.random() * maxY));

    noElement.style.left = newLeft + 'px';
    noElement.style.top = newTop + 'px';
}
